...
func (cq *CourseQuery) QueryExams() *ExamQuery {
...
func (cq *CourseQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CourseQuery) FirstIDX(ctx context.Context) uint64 {
...
func (cq *CourseQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (cq *CourseQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (cq *CourseQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (cq *CourseQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CourseQuery) WithExams(opts ...func(*ExamQuery)) *CourseQuery {
	query := (&ExamClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CourseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Course, error) {
	...
	if query := cq.withStudents; query != nil {
		if err := cq.loadStudents(ctx, query, nodes,
			func(n *Course) { n.Edges.Students = []*Student{} },
			func(n *Course, e *Student) { n.Edges.Students = append(n.Edges.Students, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withExams; query != nil {
		if err := cq.loadExams(ctx, query, nodes,
			func(n *Course) { n.Edges.Exams = []*Exam{} },
			func(n *Course, e *Exam) { n.Edges.Exams = append(n.Edges.Exams, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CourseQuery) loadStudents(ctx context.Context, query *StudentQuery, nodes []*Course, init func(*Course), assign func(*Course, *Student)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Course)
	nids := make(map[uint64]map[*Course]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(student.FieldID), joinT.C(course.StudentsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(course.StudentsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(course.StudentsPrimaryKey[1]))
...
func (cq *CourseQuery) loadExams(ctx context.Context, query *ExamQuery, nodes []*Course, init func(*Course), assign func(*Course, *Exam)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Course)
	for i := range nodes {
	...
	query.Where(predicate.Exam(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(course.ExamsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.exam_courses
		if fk == nil {
			return fmt.Errorf(`foreign-key "exam_courses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "exam_courses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CourseQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(course.Table, course.Columns, sqlgraph.NewFieldSpec(course.FieldID, field.TypeUint64))
	_spec.From = cq.sql
