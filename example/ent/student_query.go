...
func (sq *StudentQuery) QueryCourses() *CourseQuery {
...
func (sq *StudentQuery) QueryExams() *ExamQuery {
	query := (&ExamClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StudentQuery) WithCourses(opts ...func(*CourseQuery)) *StudentQuery {
	query := (&CourseClient{config: sq.config}).Query()
...
func (sq *StudentQuery) WithExams(opts ...func(*ExamQuery)) *StudentQuery {
	query := (&ExamClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := sq.withCourses; query != nil {
		if err := sq.loadCourses(ctx, query, nodes,
			func(n *Student) { n.Edges.Courses = []*Course{} },
			func(n *Student, e *Course) { n.Edges.Courses = append(n.Edges.Courses, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withExams; query != nil {
		if err := sq.loadExams(ctx, query, nodes,
			func(n *Student) { n.Edges.Exams = []*Exam{} },
			func(n *Student, e *Exam) { n.Edges.Exams = append(n.Edges.Exams, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StudentQuery) loadCourses(ctx context.Context, query *CourseQuery, nodes []*Student, init func(*Student), assign func(*Student, *Course)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Student)
	nids := make(map[uint64]map[*Student]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(student.CoursesTable)
		s.Join(joinT).On(s.C(course.FieldID), joinT.C(student.CoursesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(student.CoursesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(student.CoursesPrimaryKey[0]))
...
func (sq *StudentQuery) loadExams(ctx context.Context, query *ExamQuery, nodes []*Student, init func(*Student), assign func(*Student, *Exam)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Student)
	for i := range nodes {
	...
	for _, n := range neighbors {
		fk := n.student_exams
		if fk == nil {
			return fmt.Errorf(`foreign-key "student_exams" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
